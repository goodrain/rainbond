				...
				},
				{
					...
					Rules: []*RulesConfig{
						...
						},
						{
							...
							Expr:        "sum(node_load5) by(instance) > count by(instance)(node_cpu_seconds_total{mode=\"idle\",job=\"rbd_node\"}) *" + utils.GetenvDefault("HIGH_LOAD_ON_NODE_FACTOR", "0.7"),
							For:         "5m",
							Labels:      getCommonLabels(map[string]string{"PageAlarm": "true"}),
						...
						},
						{
							...
							Expr:        "node_filesystem_files_free{fstype=~\"ext4|xfs\"} / node_filesystem_files{fstype=~\"ext4|xfs\"} <" + utils.GetenvDefault("INODE_FREE_RATE_LOW_FACTOR", "0.3"),
							For:         "5m",
							Labels:      getCommonLabels(map[string]string{"PageAlarm": "true"}),
						...
						},
						{
							...
							Expr:        "(node_filesystem_size_bytes{mountpoint='/'} - node_filesystem_free_bytes{mountpoint='/'}) * 100 / node_filesystem_size_bytes{mountpoint='/'} >" + utils.GetenvDefault("HIGH_ROOT_DISK_USAGE_ON_NODE_GOAL", "70"),
							For:         "5m",
							Labels:      getCommonLabels(map[string]string{"PageAlarm": "true"}),
						...
						},
						{
							...
							Expr:        "(node_filesystem_size_bytes{mountpoint='/var/lib/docker'} - node_filesystem_free_bytes{mountpoint='/var/lib/docker'}) * 100 / node_filesystem_size_bytes{mountpoint='/var/lib/docker'} >" + utils.GetenvDefault("HIGH_DOCKER_DISK_USAGE_ON_NODE_GOAL", "70"),
							For:         "5m",
							Labels:      getCommonLabels(map[string]string{"PageAlarm": "true"}),
