...
if ($server_type eq "svn"){
    $cmd="tar -c --exclude=.svn ./ | $docker_bin run -i --net=host --rm --name $name -v $cache_dir:/tmp/cache:rw -a stdin -a stdout $APPEND_ENV_STRING -e SLUG_VERSION=$version -e SERVICE_ID=$service_id -e TENANT_ID=$tenant_id -e RUNTIME=$runtime -e LANGUAGE=$language -v $slug_dir:/tmp/slug goodrain.me/builder local";
    system($cmd);
}elsif ($server_type eq "git"){
    $cmd="git archive $branch | $docker_bin run -i --net=host --rm --name $name -v $cache_dir:/tmp/cache:rw -a stdin -a stdout $APPEND_ENV_STRING -e SLUG_VERSION=$version -e SERVICE_ID=$service_id -e TENANT_ID=$tenant_id -e RUNTIME=$runtime -e LANGUAGE=$language -v $slug_dir:/tmp/slug goodrain.me/builder local";
    system($cmd);
}
