...
function prepare() {
    log.info "checking kubelet service..."
}
...
#测试可用:通过代理发送请求
function is_normal() {
    log.info "checking kubelet's health..."
    api_check=$(curl 127.0.0.1:10248/healthz)
        if [[ $api_check =~ 'ok' ]];then
        log.info "service kubelet is healthy"
        return 0
        else
        log.error "service kubelet is unhealthy"
        return 1
        fi
}

function run() {
    log.info "service kubelet check"
    is_install gr-kubelet && ( 
        is_enable kubelet && ( 
            is_active kubelet && (
                is_normal && ( 
                    log.stdout '{
                        "status":[
                        {
                            "name":"check-service-kubelet",
                            "condition_type":"SERVICE_KUBELET_NORMAL",
                    ...
                    log.stdout '{
                        "status":[
                        {
                            "name":"check-kubelet-normal",
                            "condition_type":"KUBELET_IS_ABNORMAL", 
                ...
                log.stdout '{
                    "status":[
                    {
                        "name":"check-kubelet-active",
                        "condition_type":"KUBELET_ISNOT_ACTIVE", 
                        "condition_status":"False" 
                    }
                    ],
                    "exec_status":"Success", 
        ...
        log.stdout '{
            "status":[
            {
                ...
                "condition_type":"KUBELET_ISNOT_INSTALL", 
                "condition_status":"False" 
            }
            ],
            "exec_status":"Success", 
