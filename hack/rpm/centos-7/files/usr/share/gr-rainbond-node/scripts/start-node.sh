...
if [ -z $NODE_TYPE ];then
    ...
    ACP_NODE_OPTS="--log-level=debug --statsd.mapping-config=/usr/share/gr-rainbond-node/gaops/config/mapper.yml  --static-task-path=/usr/share/gr-rainbond-node/gaops/tasks/ --etcd=http://127.0.0.1:2379 --kube-conf=/etc/goodrain/kubernetes/kubeconfig --hostIP=$HOSTIP"
fi
