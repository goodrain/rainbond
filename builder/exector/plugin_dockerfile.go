...
func (e *exectorManager) runD(t *model.BuildPluginTaskBody, logger event.Logger) error {
	...
	err := sources.ImageBuild(t.Arch, sourceDir, util.GetenvDefault("RBD_NAMESPACE", constants.Namespace), t.PluginID, t.DeployVersion, logger, "plug-build", buildImageName, e.BuildKitImage, e.BuildKitArgs, e.BuildKitCache, e.KubeClient)
	if err != nil {
