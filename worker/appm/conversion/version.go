...
func updateAPISixRoute(as *v1.AppService) error {
	...
	for _, port := range ports {
		...
		for _, apisixroute := range apisixRoutes.Items {
			...
			apisixroute.Spec.HTTP[0].Name = uuid.New().String()[0:8]
		...
		for _, t := range tsmr {
			...
			if err != nil {
				if strings.Contains(err.Error(), "record not found") {
					err := dbmanager.TenantServiceMountRelationDao().DElTenantServiceMountRelationByDepService(t.ServiceID, t.DependServiceID)
					if err != nil {
						logrus.Errorf("delete relation volume failure: %v", err)
					}
					continue
				}
