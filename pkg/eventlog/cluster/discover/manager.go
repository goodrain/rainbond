...
func (d *EtcdDiscoverManager) RegisteredInstance(host string, port int, stopRegister *bool) *Instance {
	...
	success:
		...
		go func() {
			...
			_, err = d.etcdclientv3.Put(ctx, fmt.Sprintf("/traefik/backends/event_log_event_http/servers/%s/url", instance.HostID), fmt.Sprintf("%s:%d", instance.HostIP, instance.WebPort))
			if err != nil {
				d.log.Error("Register instance data to etcd discover v3 error.", err.Error())
			}
		}()
