...
// CreateStorageClass creates a storage class for the file system
func (p *VolcengineProvider) CreateStorageClass(ctx context.Context, fs *FileSystem, opts *CreateStorageClassOptions) (*storagev1.StorageClass, error) {
	if opts.Parameters == nil {
		opts.Parameters = make(map[string]string)
	}

	// Set required parameters for Volcengine NAS
	opts.Parameters["fileSystemId"] = fs.ID
	opts.Parameters["server"] = fmt.Sprintf("%s.%s.nas.volces.com", fs.ID, fs.Region)
	opts.Parameters["protocolType"] = fs.ProtocolType
	opts.Parameters["storageType"] = fs.StorageType

	var reclaimPolicy v1.PersistentVolumeReclaimPolicy
	if opts.ReclaimPolicy == "" {
		reclaimPolicy = v1.PersistentVolumeReclaimDelete
	} else {
		reclaimPolicy = v1.PersistentVolumeReclaimPolicy(opts.ReclaimPolicy)
	}

	volumeBindingMode := storagev1.VolumeBindingImmediate
	if opts.VolumeBindingMode == "WaitForFirstConsumer" {
